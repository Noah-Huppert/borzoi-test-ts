...
export default class Tester {
    ...
    async getTestResults(): Promise<TestResult> {
        ...
        let assertRes: AssertionResult[] = [];
        runtimeTester.assertions.forEach((a: Assertion, key: string) => {
            assertRes.push(a.run());
        });
        const childrenRes = await Promise.all(runtimeTester.children.map(async (c) => {
            return await c.getTestResults();
        }));
     ...
     */
    async execute() { 
        const res = await this.getTestResults();
